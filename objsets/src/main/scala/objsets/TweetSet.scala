...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("set is Empty")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet = {
      ...
      def max(tweetA: Tweet, tweetB: Tweet): Tweet =
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(tweet
  => google.exists(word => tweet.text.contains(word)))

  lazy val appleTweets: TweetSet = allTweets.filter(tweet
   ...
   */
     lazy val trending: TweetList = googleTweets.
       union(appleTweets).descendingByRetweet
  }
